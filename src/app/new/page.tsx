...
import { Filter, SortAsc, SortDesc, Grid, List, Clock, TrendingUp, Crown, Sparkles, Award, Star } from "lucide-react";
...
export default function NewArrivalsPage() {
  ...
  // Filter products that are new arrivals (created in the last 30 days)
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  
  const newArrivals = products.filter(p => new Date(p.createdAt) > thirtyDaysAgo);

  // Get unique categories and brands for new arrivals
  const categories = [...new Set(newArrivals.map(p => p.category))];
  const brands = [...new Set(newArrivals.map(p => p.brand))];

  // Filter and sort products
  const filteredProducts = useMemo(() => {
    let filtered = newArrivals.filter(product => {
      // Search filter
    ...
    return filtered;
  }, [newArrivals, searchQuery, sortBy, priceRange, selectedCategories, selectedBrands]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="w-20 h-20 bg-gradient-to-r from-gold-500 to-gold-600 rounded-3xl flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xl text-old-money-600 max-w-3xl mx-auto leading-relaxed">
                ...
                Fresh arrivals featuring the latest trends and timeless classics.
              </p>
              <div className="flex items-center justify-center gap-8 mt-8 text-lg text-old-money-500">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium">{filteredProducts.length} New Items</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-cream-200 p-6 mb-8">
              <div className="flex items-center justify-between">
                ...
                <div className="text-lg text-old-money-600 font-medium">
                  {filteredProducts.length} of {newArrivals.length} new arrivals
                </div>
              </div>
            </div>
            ...
            {filteredProducts.length > 0 ? (
                ...
                {filteredProducts.map((product) => (
                  <div key={product.id} className="relative group">
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <div className="bg-gradient-to-r from-gold-500 to-gold-600 text-white text-lg font-bold px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
                    ...
                    <div className="absolute top-4 right-4 z-10">
                      <div className="bg-gradient-to-r from-old-money-500/90 to-old-money-600/90 text-white text-sm font-semibold px-3 py-2 rounded-full shadow-lg backdrop-blur-sm">
                        {getDaysSinceCreated(product.createdAt)}d ago
                      </div>
                    </div>
                    <ProductCard 
                      product={product}
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-cream-200 p-16 text-center">
                ...
                <h3 className="text-2xl font-semibold text-old-money-800 mb-3">No new arrivals found</h3>
                <p className="text-lg text-old-money-600 mb-6 max-w-md mx-auto leading-relaxed">
                  Try adjusting your search or filter criteria to discover the latest additions to our premium collection.
                </p>
                <button
                  onClick={() => {
                    setSearchQuery("");
